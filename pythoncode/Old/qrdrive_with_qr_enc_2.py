...
def drive(coY0, coY1, coX0, coX1):
  global motor_setpoint, magnetic_heading_LSM303, ext_var
  ...
  if (math.fabs(heading_adjustment) < 10):
    heading_adjustment = 0

  print "Adjustment heading: ",
...
def encoderfeedback():
  ...
  for x_wheel in xrange(4):  
    ...
    if(encoder_reading[x_wheel] !=0 and math.fabs(motor_setpoint[x_wheel]) <= 1):
...
def read_LSM303D():
  ...
  declinationAngle =  0 - math.radians(102)
...
def main():
  ...
  global rotary_counters, lock_rotary, encoder_reading, magnetic_heading_LSM303, route_counter, ext_var
